...
void GetOnlyFileName(const char * sz_Name, std::string & strFileName)
{
	...
	int i;
	for (i=(int)(strlen(sz_Name)-1); i>=0; --i)
	{
...
bool GetLocalFileName(const char * c_szGlobalPath, const char * c_szFullPathFileName, std::string * pstrLocalFileName)
{
	...
	DWORD length = (DWORD)(min(strGlobalPath.length(), strFullPathFileName.length()));
...
void StringExceptCharacter(std::string * pstrString, const char * c_szCharacter)
{
	...
	while((iNextPos = (int)(pstrString->find_first_of(c_szCharacter, icurPos))) >= 0)
	{
...
bool SplitLine(const char * c_szLine, const char * c_szDelimeter, std::vector<std::string> * pkVec_strToken)
{
	...
	do
	{
		int beginPos = (int)(strLine.find_first_not_of(c_szDelimeter, basePos));
		if (beginPos < 0)
			return false;
		...
		{
			++beginPos;
			endPos = (int)(strLine.find_first_of("\"", beginPos));
		...
		{
			endPos = (int)(strLine.find_first_of(c_szDelimeter, beginPos));
			basePos = endPos;
		}
