...
bool CMemoryTextFileLoader::SplitLineByTab(DWORD dwLine, CTokenVector* pstTokenVector)
{
	...
	const int c_iLineLength = (int)(c_rstLine.length());
	...
	do
	{
		int beginPos = (int)(c_rstLine.find_first_of("\t", basePos));
...
int CMemoryTextFileLoader::SplitLine2(DWORD dwLine, CTokenVector* pstTokenVector, const char * c_szDelimeter)
{
	...
	do
	{
		int beginPos = (int)(c_rstLine.find_first_not_of(c_szDelimeter, basePos));
		...
		{
			++beginPos;
			endPos = (int)(c_rstLine.find_first_of("\"", beginPos));
		...
		{
			endPos = (int)(c_rstLine.find_first_of(c_szDelimeter, beginPos));
			basePos = endPos;
		}
...
{
	return (DWORD)(m_stLineVector.size());
}
...
bool CDiskFileLoader::Read(int size, void* pvDst)
{
	...
	int ret = (int)(fread(pvDst, size, 1, m_fp));
