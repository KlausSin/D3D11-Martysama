...
// DDS color key (not used but needed for struct layout)
struct DDCOLORKEY
{
...
// DDS surface description - the main DDS file header
...
static_assert(sizeof(DDSURFACEDESC2) == 124, "DDS header must match the 124 byte on-disk layout");
...
bool CDXTCImage::LoadHeaderFromMemory(const BYTE* c_pbMap, int iSize)
{
	...
	// Validate DDS magic number - "DDS " (0x20534444)
	...
	m_dwMipMapCount = max(1u, ddsd.dwMipMapCount);
	...
	else
	{
		...
		{
			DWORD minBlockSize = (m_CompFormat == PF_DXT1) ? 8 : 16;
			if (m_CompFormat == PF_ARGB)
				minBlockSize = 4; // 1 pixel minimum

			for (DWORD dwLinearSize = ddsd.dwLinearSize, i = 0; i < m_dwMipMapCount; ++i, dwLinearSize >>= 2)
			{
				m_pbCompBufferByLevels[i] = c_pbMap;
				c_pbMap += max(dwLinearSize, minBlockSize);
			}
		}
		else
		{
...
bool CDXTCImage::LoadFromMemory(const BYTE* c_pbMap, int iSize)
{
	...
	else
	{
		...
		if (m_dwFlags & DDSD_MIPMAPCOUNT)
		{
			...
			{
				DWORD actualSize = max(dwLinearSize, minBlockSize);
				m_bCompVector[i].resize(actualSize);
				Copy(i, &m_bCompVector[i][0], actualSize);
			}
		}
		else
		{
			DWORD actualSize = max((DWORD)m_lPitch, minBlockSize);
			m_bCompVector[0].resize(actualSize);
			Copy(0, &m_bCompVector[0][0], actualSize);
		}
	}
...
bool CDXTCImage::Copy(int miplevel, BYTE * pbDest, long lDestPitch)
{
	...
	if (miplevel >= (int)m_dwMipMapCount)
		return false;

	if (!m_pbCompBufferByLevels[miplevel])
		return false;

	// Minimum block size depends on format
	long minBlockSize = (m_CompFormat == PF_DXT1) ? 8 : 16;
	if (m_CompFormat == PF_ARGB)
		minBlockSize = 4;

	long copySize = m_lPitch >> (miplevel * 2);
	if (copySize < minBlockSize)
		copySize = minBlockSize;

	if (copySize > lDestPitch)
		copySize = lDestPitch;

	memcpy(pbDest, m_pbCompBufferByLevels[miplevel], copySize);
	return true;
}
...
{
	// Safety check: ensure mip level is valid and has data
	if (miplevel < 0 || miplevel >= MAX_MIPLEVELS)
		return;
	if (m_bCompVector[miplevel].empty())
		return;
	if (!pdwDest)
		return;
