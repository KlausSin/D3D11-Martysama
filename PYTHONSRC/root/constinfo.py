...
# Volumetric fog dials. Density and height persist across maps; the fog LEVEL buttons drive the
# volume's far extent so the three existing presets keep meaning for the new fog too.
# Two fog dials, 0..4 multipliers, 1.0 = default (same model as the DX12 client).
#   VFOG_DENSITY = thickness (0..4)       VFOG_START = distance where fog begins (world units)
VFOG_DENSITY = 1.0
VFOG_START   = 0.0

def APPLY_VOLUMETRIC_FOG():
	global VFOG_DENSITY, VFOG_START
	try:
		app.SetVolumetricFogDensity(VFOG_DENSITY)
		app.SetVolumetricFogRange(VFOG_START, 0.0)
	except:
		pass

def SET_VOLUMETRIC_FOG_DENSITY(d):
	global VFOG_DENSITY
	VFOG_DENSITY = d
	APPLY_VOLUMETRIC_FOG()

def SET_VOLUMETRIC_FOG_START(v):
	global VFOG_START
	VFOG_START = v
	APPLY_VOLUMETRIC_FOG()
