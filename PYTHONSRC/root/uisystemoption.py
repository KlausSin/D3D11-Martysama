...
VFOG_DENSITY_MAX = 4.0
VFOG_START_MAX   = 8000.0
...
class OptionDialog(ui.ScriptWindow):
	...
	def __Load_BindObject(self):
		try:
			...
			self.ctrlFogDensity = GetObject("vfog_density_controller")
			self.ctrlFogStart = GetObject("vfog_start_controller")
...
#		self.ctrlShadowQuality.SetEvent(ui.__mem_func__(self.OnChangeShadowQuality))
		...
		self.ctrlFogDensity.SetSliderPos(constInfo.VFOG_DENSITY / VFOG_DENSITY_MAX)
		self.ctrlFogDensity.SetEvent(ui.__mem_func__(self.OnChangeFogDensity))

		self.ctrlFogStart.SetSliderPos(constInfo.VFOG_START / VFOG_START_MAX)
		self.ctrlFogStart.SetEvent(ui.__mem_func__(self.OnChangeFogStart))
	...
	def OnChangeFogDensity(self):
		pos = self.ctrlFogDensity.GetSliderPos()
		constInfo.SET_VOLUMETRIC_FOG_DENSITY(pos * VFOG_DENSITY_MAX)

	def OnChangeFogStart(self):
		pos = self.ctrlFogStart.GetSliderPos()
		constInfo.SET_VOLUMETRIC_FOG_START(pos * VFOG_START_MAX)
