...
namespace FoxFS
{
	...
	int FileSystem::setKeyServer(const char* host, unsigned int port)
	{
		...
		else
		{
			...
			else
			{
				...
				if (err > 0 && FD_ISSET(s, &fdWrite))
				{
					...
					reinterpret_cast<unsigned short*>(authrequest)[0] = static_cast<unsigned short>(hwidlen);
					...
					else
					{
						...
						else
						{
							...
							while (offset < datalen && offset < (8192 * 2))
							{
								...
								offset += (unsigned int)(curname.length() + 1);
								struct KeyInfo info;
		...
		if (wnd)
		{
			unsigned int dif = static_cast<unsigned int>(time(0) - windowCreation);
			if (dif <= 5000)
			{
				Sleep(dif);
			}
