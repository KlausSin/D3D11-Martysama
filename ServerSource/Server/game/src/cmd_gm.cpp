...
ACMD(do_mob)
{
	...
	// Raised cap from 20/40 → 30000 to support stress-testing the GPU-driven
	// rendering path. The retry loop below keeps the effective spawn count
	// bounded by attempts, not by a hard cap.
	iCount = MINMAX(1, iCount, 30000);
	const int iRequested = iCount;
	int iSpawned = 0;

	// SpawnMobRange fails often when the random position lands on water, a
	// wall, off the map, or too close to another entity. Without retries the
	// caller gets only a fraction of what they asked for. Retry until we hit
	// the requested count or burn through a fixed budget — 20× the request is
	// generous enough to cover bad-terrain maps without running forever.
	const int iAttemptBudget = iRequested * 20;
	int iAttempts = 0;
	while (iSpawned < iRequested && iAttempts < iAttemptBudget)
	{
		++iAttempts;
	...
	ch->ChatPacket(CHAT_TYPE_INFO, "Spawned %d/%d %s (vnum %u) in %d attempts",
		iSpawned, iRequested, pkMob->m_table.szLocaleName, vnum, iAttempts);
}
