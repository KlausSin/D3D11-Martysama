...
// Chinese Traditional
#define _CHT_HKL_DAYI					((HKL)(UINT_PTR)0xE0060404)	// DaYi
#define _CHT_HKL_NEW_PHONETIC			((HKL)(UINT_PTR)0xE0080404)	// New Phonetic
#define _CHT_HKL_NEW_CHANG_JIE			((HKL)(UINT_PTR)0xE0090404)	// New Chang Jie
#define _CHT_HKL_NEW_QUICK				((HKL)(UINT_PTR)0xE00A0404)	// New Quick
#define _CHT_HKL_HK_CANTONESE			((HKL)(UINT_PTR)0xE00B0404)	// Hong Kong Cantonese
...
// Chinese Simplized
#define _CHS_HKL						((HKL)(UINT_PTR)0xE00E0804) // MSPY
#define _CHS_HKL_QQPINYIN				((HKL)(UINT_PTR)0xE0210804) // QQ PinYin
#define _CHS_HKL_SOGOU					((HKL)(UINT_PTR)0xE0220804) // Sougou PinYin
#define _CHS_HKL_GOOGLEPINYIN			((HKL)(UINT_PTR)0xE0230804) // Google PinYin
...
int CIME::GetReading(std::string & rstrText)
{
	...
	if(ms_wstrReading.size() == 0)
		return 0;
	int readingLen = WideCharToMultiByte(ms_uOutputCodePage, 0, &ms_wstrReading[0], (int)(ms_wstrReading.size()), reading, sizeof(reading), NULL, NULL);

	rstrText.append(GetCodePageText());
	rstrText.append(reading, reading + readingLen);

	return (int)(rstrText.size());
}
...
int  CIME::GetCandidate(DWORD index, std::string & rstrText)
{
	...
	int wTextLen = (int)(wcslen(wszText));
	if(wTextLen == 0)
		return 0;
...
void CIME::CandidateProcess(HIMC hImc)
{
	...
	if(dwCandidateLen > 0) {
		...
		if( GETLANG() == LANG_CHS ) {
			...
			{
				UINT uLen = lstrlenW((LPWSTR)((BYTE*)lpCandidateList + lpCandidateList->dwOffset[i])) + (3 - sizeof(WCHAR));
	    ...
	    for(UINT i = iStartOfPage, j = 0; (DWORD)i < lpCandidateList->dwCount && j < ms_dwCandidatePageSize; i++, j++) {
			wcscpy( ms_wszCandidate[j], (LPWSTR)( (BYTE*)lpCandidateList + lpCandidateList->dwOffset[i] ) );
	    }
...
bool CIME::IsMax(const wchar_t* wInput, int len)
{
	...
	else if (m_userMax != 0 && m_max != m_userMax)
	{
		...
		int textLen = WideCharToMultiByte(ms_uOutputCodePage, 0, str.c_str(), (int)(str.length()), 0, 0, NULL, NULL);
		int inputLen = WideCharToMultiByte(ms_uOutputCodePage, 0, input.c_str(), (int)(input.length()), 0, 0, NULL, NULL);
...
DWORD CIME::GetImeId( UINT uIndex )
{
	...
	DWORD dwLang = ((DWORD)(DWORD_PTR)hkl & 0xffff);
...
void CIME::CheckToggleState()
{
	...
	bool bIme = ImmIsIME( ms_hklCurrent ) != 0
		&& ( ( 0xF0000000 & (DWORD)(DWORD_PTR)ms_hklCurrent ) == 0xE0000000 ); // Hack to detect IME correctly. When IME is running as TIP, ImmIsIME() returns true for CHT US keyboard.
