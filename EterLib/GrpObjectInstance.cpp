...
void CGraphicObjectInstance::Clear()
{
	...
	MatrixIdentity(&m_matAbsoluteTrans);
	MatrixIdentity(&m_matScale);
	MatrixIdentity(&m_matScaleWorld);
#endif

	ZeroMemory(m_abyPortalID, sizeof(m_abyPortalID));

#ifdef ENABLE_FRUSTUM_CULLING
	m_fBoundingSphereRadius = 200.0f;  // Default radius
#endif
...
{
	// No visibility check — shadow casters must render regardless of camera frustum
	OnRenderToShadowMap();
}
...
{
	// Skip matrix recalculation if not dirty (optimization)
	if (!m_bMatrixDirty)
		return;
...
void CGraphicObjectInstance::SetScaleWorld(float x, float y, float z)
{
	...
	m_v3Scale.y = y;
	m_v3Scale.z = z;
	MatrixScaling(&m_matScaleWorld, x, y, z);
	m_bMatrixDirty = true;
}
...
void CGraphicObjectInstance::SetAcceScale(const Vector3& rv3Scale, BYTE bRace)
{
...
Matrix & CGraphicObjectInstance::GetMatrix()
{
...
{
	MatrixRotationQuaternion(&m_mRotation, &q);
	m_bMatrixDirty = true;
}
...
void CGraphicObjectInstance::SetRotation(float fRotation)
{
	...
	MatrixRotationZ(&m_mRotation, ToRadian(fRotation));
	m_bMatrixDirty = true;
}
...
void CGraphicObjectInstance::SetRotation(float fYaw, float fPitch, float fRoll)
{
	...
	MatrixRotationYawPitchRoll(&m_mRotation, ToRadian(fYaw), ToRadian(fPitch), ToRadian(fRoll));
	m_bMatrixDirty = true;
}
...
void CGraphicObjectInstance::SetPosition(const Vector3& rv3Pos)
{
	m_v3Position = rv3Pos;
	m_bMatrixDirty = true;
}
...
#ifdef ENABLE_FRUSTUM_CULLING
	m_fBoundingSphereRadius = 200.0f;
#endif
...
{
	return (DWORD)(m_StaticCollisionInstanceVector.size());
}
