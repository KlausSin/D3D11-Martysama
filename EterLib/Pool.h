...
class CDynamicPool
{
	public:
		...
		void Compact()
		{
			if (m_kVct_pkFree.empty())
				return;
			std::unordered_set<T*> kFreeSet(m_kVct_pkFree.begin(), m_kVct_pkFree.end());
			std::vector<T*> kKept;
			kKept.reserve(m_kVct_pkData.size() - m_kVct_pkFree.size());
			for (T* p : m_kVct_pkData)
			{
				if (kFreeSet.count(p))
					Delete(p);
				else
			...
			m_kVct_pkData.swap(kKept);
			m_kVct_pkFree.clear();
			m_uUsedCapacity = (UINT)m_kVct_pkData.size();
		}

		DWORD GetCapacity()
		{
			return (DWORD)(m_kVct_pkData.size());
		}
