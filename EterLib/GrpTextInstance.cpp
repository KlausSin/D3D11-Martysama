...
#ifdef ENABLE_IMGUI_MANAGER
// Color parsing helper structures for FreeType rendering
...
static std::vector<ColoredTextSegment> ParseColorTags(const std::string& text, DWORD defaultColor, int codePage)
{
	std::vector<ColoredTextSegment> segments;
	std::string currentText;
	DWORD currentColor = defaultColor;
	...
	while (pos < text.length())
	{
		...
		if (text[pos] == '|' && pos + 1 < text.length())
		{
			...
			// Color tag: |cFFRRGGBB
			if (nextChar == 'c' && pos + 10 <= text.length())
			{
				// Save current segment if any
				if (!currentText.empty())
				{
					int wTextMax = static_cast<int>(currentText.length()) * 2;
					std::vector<wchar_t> wText(wTextMax);
					int wTextLen = MultiByteToWideChar(codePage, 0, currentText.c_str(),
						static_cast<int>(currentText.length()), wText.data(), wTextMax);

					if (wTextLen > 0)
					{
						ColoredTextSegment seg;
						seg.text = std::wstring(wText.data(), wTextLen);
				...
				// Parse color: |cFFRRGGBB (10 characters total)
				std::string colorStr = text.substr(pos + 2, 8);
				try {
					currentColor = std::stoul(colorStr, nullptr, 16);
				}
				catch (...) {
			...
			// Hyperlink tags: |h or |H - just skip
...
int CGraphicTextInstance::Hyperlink_GetText(char* buf, int len)
{
	...
	return Ymir_WideCharToMultiByte(codePage, 0, gs_hyperlinkText.c_str(), (int)(gs_hyperlinkText.length()), buf, len, NULL, NULL);
}
...
{
	index = std::min<DWORD>(index, (DWORD)(m_pCharInfoVector.size()));
...
{
	char retBuf[1024];
	const int retLen = Ymir_WideCharToMultiByte(GetDefaultCodePage(), 0, emojiBuffer.c_str(), (int)(emojiBuffer.length()), retBuf, sizeof(retBuf) - 1, nullptr, nullptr);
	retBuf[retLen] = '\0';
...
void CGraphicTextInstance::Update()
{
	...
	const size_t estimatedCharCount = m_stText.length();
	m_pCharInfoVector.reserve(estimatedCharCount);
	m_dwColorInfoVector.reserve(estimatedCharCount);
	...
	while (begin < end)
	{
		...
		else
		{
			if (defCodePage == CP_ARABIC) // ARABIC
			{
				...
				int wArabicTextLen = (int)(Arabic_MakeShape(wText, wTextLen, wArabicText, wTextLen));
...
void CGraphicTextInstance::Render(RECT * pClipRect)
{
...
#ifdef ENABLE_IMGUI_MANAGER
	// Use FreeType/ImGui for text rendering
	if (CImGuiManager::Instance().IsInitialized())
	{
		...
		const CImGuiManager::ERenderLayer layer = CImGuiManager::ERenderLayer::Background;

		// Parse color tags from text
		std::vector<ColoredTextSegment> segments = ParseColorTags(m_stText, m_dwTextColor, codePage);

		if (segments.empty() && !m_isCursor)
			return;
		...
		if (m_isSecret)
		{
			...
			ColoredTextSegment seg;
			int totalChars = 0;
			for (size_t i = 0; i < m_stText.length(); ++i)
			{
				if ((m_stText[i] & 0x80) == 0) totalChars++;
				else if ((m_stText[i] & 0xE0) == 0xC0) { totalChars++; i++; }
				else if ((m_stText[i] & 0xF0) == 0xE0) { totalChars++; i += 2; }
			}
			seg.text = std::wstring(totalChars, L'*');
			seg.color = m_dwTextColor;
			seg.offsetX = 0.0f;
			segments.push_back(seg);
		}
		...
		for (auto& seg : segments)
		{
			if (!seg.text.empty())
			{
				int segWidth = 0, segHeight = 0;
				CImGuiManager::Instance().GetTextExtentW(seg.text, &segWidth, &segHeight);
				seg.offsetX = static_cast<float>(totalWidth);
				totalWidth += segWidth;
				if (segHeight > textHeight)
					textHeight = segHeight;
			}
		}
		...
		if (codePage == CP_ARABIC)
		{
			if (m_hAlign == HORIZONTAL_ALIGN_LEFT)
				fStanX -= totalWidth;
			else if (m_hAlign == HORIZONTAL_ALIGN_CENTER)
				fStanX -= totalWidth / 2.0f;
		}
		else
		{
			if (m_hAlign == HORIZONTAL_ALIGN_RIGHT)
				fStanX -= totalWidth;
			else if (m_hAlign == HORIZONTAL_ALIGN_CENTER)
				fStanX -= totalWidth / 2.0f;
		}
		...
		// Get draw list for clipping
		ImDrawList* drawList = ImGui::GetBackgroundDrawList();

		if (pClipRect)
		{
			drawList->PushClipRect(
				ImVec2(static_cast<float>(pClipRect->left), static_cast<float>(pClipRect->top)),
				ImVec2(static_cast<float>(pClipRect->right), static_cast<float>(pClipRect->bottom)),
				true
			);
		}
		...
		for (const auto& seg : segments)
		{
			...
			float segX = fStanX + seg.offsetX;
			float segY = fStanY;

			if (m_isOutline)
			{
				CImGuiManager::Instance().RenderTextWithOutlineW(
					seg.text, segX, segY, seg.color, m_dwOutLineColor, layer);
			}
			else
			{
				CImGuiManager::Instance().RenderTextW(
					seg.text, segX, segY, seg.color, false, layer);
			}
		}
		...
		// Handle cursor rendering for input fields
		if (m_isCursor && textHeight > 0)
		{
			...
			float sy = fStanY;
			float ex = sx + 2;
			float ey = sy + textHeight;

			int curpos = CIME::GetCurPos();
			if (curpos > 0 && static_cast<size_t>(curpos) <= fullText.length())
			{
				std::wstring textBeforeCursor = fullText.substr(0, curpos);
				int cursorX = 0, cursorY = 0;
				CImGuiManager::Instance().GetTextExtentW(textBeforeCursor, &cursorX, &cursorY);
				sx = fStanX + cursorX;
				ex = sx + 2;
			}
		...
		// Flush to ensure correct render order
		CImGuiManager::Instance().FlushAndRestart();

		return; // Done with ImGui rendering
	}
#endif

	// ===== ORIGINAL RENDERING CODE (fallback) =====
	...
	static std::unordered_map<CGraphicImageTexture*, std::vector<SPDTVertexRaw>> verticesMap;
	for (auto& [pTex, vtxBatch] : verticesMap)
		vtxBatch.clear();
#endif
	{
		...
		if (m_isOutline)
		{
			...
			for (i=m_pCharInfoVector.begin(); i!=m_pCharInfoVector.end(); ++i)
			{
				...
				SHADERMANAGER.SetShaderResource(0, pFontTexture->GetD3DTexture());
#endif
	...
	for (auto& p : verticesMap)
	{
		...
		SHADERMANAGER.SetShaderResource(0, p.first->GetD3DTexture());

		static const size_t MAX_BATCH_QUADS = 512;
		size_t totalQuads = p.second.size() / 4;
		size_t offset = 0;

		while (totalQuads > 0)
		{
			size_t batchQuads = (totalQuads > MAX_BATCH_QUADS) ? MAX_BATCH_QUADS : totalQuads;
			UINT vtxCount = (UINT)(batchQuads * 4);

			if (CGraphicBase::SetPDTStream(&p.second[offset], vtxCount))
			{
				CGraphicBase::SetDefaultIndexBuffer(CGraphicBase::DEFAULT_IB_FILL_RECT_MULTI);
				SHADERMANAGER.DrawIndexed(TOPOLOGY_TRIANGLELIST, 0, vtxCount, 0, (UINT)(batchQuads * 2));
			}

			offset += vtxCount;
			totalQuads -= batchQuads;
		}
	}
#endif
	...
	if (m_isCursor)
	{
		...
		if(ulbegin < ulend)
		{
			...
			SHADERMANAGER.DrawIndexedDynamic(TOPOLOGY_TRIANGLELIST, 0, 4, 2, c_FillRectIndices, DXGI_FORMAT_R16_UINT, vertices, sizeof(TPDTVertex));
		}
	}
...
{
	m_dwOutLineColor=Color(r, g, b, a);
}
