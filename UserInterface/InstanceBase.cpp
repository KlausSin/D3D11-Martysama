...
void CInstanceBase::SHORSE::RenderWithoutStateSetup()
{
	if (!IsMounting())
		return;

	CActorInstance& rkActor=GetActorRef();
	rkActor.RenderWithOneTexture();
}

void CInstanceBase::SHORSE::BlendRenderWithoutStateSetup()
{
	if (!IsMounting())
		return;

	CActorInstance& rkActor=GetActorRef();
	rkActor.BlendRenderWithOneTexture();
}
...
bool CInstanceBase::__IsInDustRange()
{
	...
	float fDistance = m_fCachedDistanceFromMain;
...
void CInstanceBase::__Create_SetName(const SCreateData& c_rkCreateData)
{
	...
	if (IsEnemy() && CPythonSystem::Instance().IsShowMobAIFlag() && CPythonNonPlayer::Instance().IsAggressive(GetRace()))
	{
		...
		strName += "*";
		SetNameString(strName.c_str(), (int)(strName.length()));
	}
	else
		SetNameString(c_rkCreateData.m_stName.c_str(), (int)(c_rkCreateData.m_stName.length()));
#else
...
void CInstanceBase::__Create_SetWarpName(const SCreateData& c_rkCreateData)
{
	...
	if (CPythonNonPlayer::Instance().GetName(c_rkCreateData.m_dwRace, &c_szName))
	{
		...
		int iFindingPos = (int)(strName.find_first_of(" ", 0));
		if (iFindingPos > 0)
		{
...
void CInstanceBase::StateProcess()
{
	while (1)
	{
		...
		*/
		...
		float fDirLenSq = kPPosDir.x * kPPosDir.x + kPPosDir.y * kPPosDir.y;
...
void CInstanceBase::MovementProcess()
{
	...
	{
		const Vector3 & c_rkV3Mov = m_GraphicThingInstance.GetMovementVectorRef();
	...
	float fCurLenSq = kPPosDeltaSC.x * kPPosDeltaSC.x + kPPosDeltaSC.y * kPPosDeltaSC.y;
	float fNextLenSq = kPPosDeltaSN.x * kPPosDeltaSN.x + kPPosDeltaSN.y * kPPosDeltaSN.y;
	float fTotalLenSq = kPPosDeltaSD.x * kPPosDeltaSD.x + kPPosDeltaSD.y * kPPosDeltaSD.y;

	float fCurLen = 0.0f, fNextLen = 0.0f, fTotalLen = 0.0f, fRestLen = 0.0f;
	if (m_isGoing && IsWalking())
	{
		fCurLen = sqrtf(fCurLenSq);
		fNextLen = sqrtf(fNextLenSq);
		fTotalLen = sqrtf(fTotalLenSq);
		fRestLen = fTotalLen - fCurLen;
	}
...
static DWORD s_dwAnimLODFrameCounter = 0;
#endif
...
void CInstanceBase::Update()
{
	...
	if (!__IsMainInstance())
	{
		...
		if (pkInstMain)
		{
			...
			{
				// Far distance: update motion every N frames
				if ((s_dwAnimLODFrameCounter % ANIMATION_LOD_SKIP_FRAMES_FAR) != (GetVirtualID() % ANIMATION_LOD_SKIP_FRAMES_FAR))
					bProcessMotion = false;
			}
			else if (fDistance > ANIMATION_LOD_DISTANCE_HIGH)
			{
				// Mid distance: update motion every N frames
				if ((s_dwAnimLODFrameCounter % ANIMATION_LOD_SKIP_FRAMES_MID) != (GetVirtualID() % ANIMATION_LOD_SKIP_FRAMES_MID))
					bProcessMotion = false;
			}
		}
	}
...
void CInstanceBase::Transform()
{
	...
	else
	{
		if (IsWalking() || m_GraphicThingInstance.IsUsingMovingSkill())
		{
			const Vector3& c_rv3Movment=m_GraphicThingInstance.GetMovementVectorRef();
...
void CInstanceBase::RenderBlendPassDeferred()
{
	...
	switch (rkActor.GetRenderMode())
	{
		case CActorInstance::RENDER_MODE_BLEND:
			if (1.0f == rkActor.GetAlphaValue())
			{
				rkActor.BeginOpacityRender();
				rkActor.BlendRenderWithOneTexture();
				rkActor.EndOpacityRender();
			}
			else if (rkActor.GetAlphaValue() > 0.0f)
			{
				rkActor.BeginBlendRender();
...
void CInstanceBase::RenderWithoutStateSetup()
{
	if (!__CanRender())
		return;
	...
	m_kHorse.RenderWithoutStateSetup();
	m_GraphicThingInstance.RenderWithOneTexture();
}

void CInstanceBase::BlendRenderWithoutStateSetup()
{
	if (!__CanRender())
		return;

	m_kHorse.BlendRenderWithoutStateSetup();
	m_GraphicThingInstance.BlendRenderWithOneTexture();
}
...
DWORD CInstanceBase::GetAddColor()
{
	return m_GraphicThingInstance.GetAddColorDWORD();
}
#endif
...
void CInstanceBase::SetAddColor(const Color & c_rColor)
{
...
void CInstanceBase::__Initialize()
{
	...
	m_fCachedDistanceFromMain = 0.0f;
	m_dwCachedDistanceFrame = 0;
