...
int SplitLine(const char * c_szText, CTokenVector* pstTokenVector, const char * c_szDelimeter)
{
	...
	do
	{
		int beginPos = (int)(stLine.find_first_not_of(c_szDelimeter, basePos));
		...
		{
			++beginPos;
			endPos = (int)(stLine.find_first_of("\"", beginPos));
		...
		{
			endPos = (int)(stLine.find_first_of(c_szDelimeter, beginPos));
			basePos = endPos;
		}
	...
	for (DWORD i = 0; i < textFileLoader.GetLineCount(); ++i)
	{
		...
		if (!rSkillData.AffectDataVector.empty() && USE_SKILL_PROTO)
		{
				...
				strstr(c_strPointPoly.c_str(), "number")	!= NULL)
			{
				...
				// MIN
				string_replace_word(c_strPointPoly.c_str(), (int)(c_strPointPoly.length()),
					"number", 6, "min", 3, src_poly_rand);
				string_replace_word(src_poly_rand.c_str(), (int)(src_poly_rand.length()),
					"atk", 3, "minatk", 6, src_poly_atk);
				string_replace_word(src_poly_atk.c_str(), (int)(src_poly_atk.length()),
...
bool CPythonSkill::RegisterSkill(DWORD dwSkillIndex, const char * c_szFileName)
{
...

	{
		...
		if (isConditionData)
		{
			DWORD dwSize = (DWORD)(pConditionDataVector->size());
...

	{
		...
		if (isAffectData)
		{
			DWORD dwSize = (DWORD)(pAffectDataVector->size()/3);
...
void CPythonSkill::TEST()
{
	...
	for (TSkillDataMap::iterator itor = m_SkillDataMap.begin(); itor != m_SkillDataMap.end(); ++itor)
	{
		...
		int iPos = (int)(strFileName.find_last_of("/", rSkillData.strIconFileName.length()));
		if (iPos > 0)
...
const char * CPythonSkill::SSkillData::GetAffectDescription(DWORD dwIndex, float fSkillLevel)
{
	...
	if (CP_ARABIC == ::GetDefaultCodePage())
	{
		...
		int first = (int)(strDescription.find("%.0f"));
		if (first >= 0)
		{
			...
			int second = (int)(strDescription.find("%.0f", first));
			if (second >= 0)
			{
...
PyObject * skillGetIconImage(PyObject * poSelf, PyObject * poArgs)
{
	...
	return Py_BuildValue("L", (long long)c_pSkillData->pImage);
}
...
PyObject * skillGetIconInstance(PyObject * poSelf, PyObject * poArgs)
{
	...
	return Py_BuildValue("L", (long long)pImageInstance);
}
...
PyObject * skillGetIconImageNew(PyObject * poSelf, PyObject * poArgs)
{
	...
	return Py_BuildValue("L", (long long)c_pSkillData->GradeData[iGradeIndex].pImage);
}
