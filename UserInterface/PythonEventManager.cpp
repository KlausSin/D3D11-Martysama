...
void CPythonEventManager::__InitEventSet(TEventSet& rEventSet)
{
	...
	rEventSet.CurrentColor = Color(1, 1, 1, 1);
	...
	rEventSet.DiffuseColor = Color(1, 1, 1, 1);
...
DWORD CPythonEventManager::GetEmptyEventSetSlot()
{
	...
	return (DWORD)(m_EventSetVector.size()-1);
}
...
bool CPythonEventManager::ProcessEventSet(TEventSet * pEventSet)
{
	...
	switch (iEventType)
	{
		case EVENT_TYPE_LETTER:
		{
			...
			pEventSet->iCurrentLetter+=(int)(c_rstValue.length());
		...
		case EVENT_TYPE_ITEM_NAME:
		{
			...
			if (CItemManager::Instance().GetItemDataPointer(iIndex, &pItemData))
			{
				...
				pEventSet->iCurrentLetter+= (int)(strlen(pItemData->GetName()));
		...
		case EVENT_TYPE_MONSTER_NAME:
		{
			...
			if (rkNonPlayer.GetName(iIndex, &c_szName))
			{
				...
				pEventSet->pCurrentTextLine->SetValue(pEventSet->strCurrentLine.c_str());
				pEventSet->iCurrentLetter+= (int)(strlen(c_szName));
...
void CPythonEventManager::MakeQuestion(TEventSet * pEventSet, script::TArgList & rArgumentList)
{
	...
	pEventSet->nAnswer = (int)(rArgumentList.size());
...
int CPythonEventManager::GetProcessedLineCount(int iIndex)
{
	...
	return (int)(pEventSet->ScriptTextLineList.size());
}
...
int CPythonEventManager::GetLineCount(int iIndex)
{
	...
	TEventSet * pEventSet = m_EventSetVector[iIndex];
	return (int)(pEventSet->ScriptTextLineList.size()+pEventSet->iAdjustLine);
}
