...
bool CGuildMarkUploader::__Load(const char* c_szFileName, UINT* peError)
{
	...
	// Convert RGBA to BGRA and copy to mark buffer
	Pixel* srcPixels = (Pixel*)data;
	for (int i = 0; i < SGuildMark::SIZE; ++i)
	{
		BYTE* p = (BYTE*)&srcPixels[i];
		// Swap R and B (RGBA -> BGRA)
		BYTE temp = p[0];
		p[0] = p[2];
		p[2] = temp;
		m_kMark.m_apxBuf[i] = srcPixels[i];
	}
...
{
	// Validate image dimensions using stb_image
	int width, height, channels;
	unsigned char* data = stbi_load(c_szFileName, &width, &height, &channels, 4);
...
#endif
	kPacketSymbolUpload.size=(WORD)(sizeof(TPacketCGSymbolUpload) + m_dwSymbolBufSize);
