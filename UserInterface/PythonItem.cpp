...
{
	char szInfo[256];
	sprintf(szInfo, "Item: Inst %d, Pool %d", (int)m_GroundItemInstanceMap.size(), (int)m_GroundItemInstancePool.GetCapacity());
...
void CPythonItem::TGroundItemInstance::__PlayDropSound(DWORD eItemType, const Vector3& c_rv3Pos)
{
...
bool CPythonItem::TGroundItemInstance::Update()
{
	...
	if (dwEndTime < DX::StepTimer::instance().GetTotalMillieSeconds())
	{
		...
		ToRadian(rEnd.x),
		ToRadian(rEnd.z));
		Vec3TransformCoord(&v3Adjust,&v3Adjust,&mat);*/

		Quaternion qAdjust(-v3Center.x, -v3Center.y, -v3Center.z, 0.0f);
		Quaternion qc;
		QuaternionConjugate(&qc, &qEnd);
		QuaternionMultiply(&qAdjust,&qAdjust,&qEnd);
		QuaternionMultiply(&qAdjust,&qc,&qAdjust);
	...
	else
	{
		...
		Vector3 v3NewPosition=v3EndPosition;// = rate*(v3EndPosition - v3StartPosition) + v3StartPosition;
		v3NewPosition.z += 100-100*rate*(3*rate-2);//-100*(rate-1)*(3*rate+2);

		Quaternion q;
		QuaternionRotationAxis(&q, &v3RotationAxis, etime * 0.03f *(-1+rate*(3*rate-2)));
		//ThingInstance.SetRotation(rEnd.y + etime*rStart.y, rEnd.x + etime*rStart.x, rEnd.z + etime*rStart.z);
		QuaternionMultiply(&q,&qEnd,&q);

		ThingInstance.SetRotationQuaternion(q);
		Quaternion qAdjust(-v3Center.x, -v3Center.y, -v3Center.z, 0.0f);
		Quaternion qc;
		QuaternionConjugate(&qc, &q);
		QuaternionMultiply(&qAdjust,&qAdjust,&q);
		QuaternionMultiply(&qAdjust,&qc,&qAdjust);
		...
		Matrix mat;
		MatrixRotationYawPitchRoll(&mat,
		ToRadian(rEnd.y + etime*rStart.y),
		ToRadian(rEnd.x + etime*rStart.x),
		ToRadian(rEnd.z + etime*rStart.z));
...
void CPythonItem::Render()
{
	...
	if (VTFMANAGER.IsInitialized() && m_GroundItemInstanceMap.size() >= 3)
	{
		RenderVTFBatched();
		return;
	}
...
void CPythonItem::RenderVTFBatched()
{
	struct SItemVTFInfo
	{
	...
	struct SItemBatchKey
	{
		CGrannyModel* pModel;
		ID3D11ShaderResourceView* pFirstTex;
		bool operator==(const SItemBatchKey& o) const { return pModel == o.pModel && pFirstTex == o.pFirstTex; }
	};
	struct SItemBatchKeyHash
	{
		size_t operator()(const SItemBatchKey& k) const { return std::hash<void*>()(k.pModel) ^ (std::hash<void*>()(k.pFirstTex) << 1); }
	};
	std::unordered_map<SItemBatchKey, std::vector<SItemVTFInfo>, SItemBatchKeyHash> vtfGroups;
	std::vector<TGroundItemInstance*> nonBatchable;

	for (auto itor = m_GroundItemInstanceMap.begin(); itor != m_GroundItemInstanceMap.end(); ++itor)
	{
		TGroundItemInstance* pItem = itor->second;

		if (!pItem->bAnimEnded)
		{
			nonBatchable.push_back(pItem);
			continue;
		}

		CGraphicThingInstance& rThingInst = pItem->ThingInstance;

		bool bBatchable = false;
		if (rThingInst.GetLODControllerCount() >= 1)
		{
			CGrannyLODController* pLOD = rThingInst.GetLODControllerPointer(0);
			if (pLOD && pLOD->isModelInstance())
			{
				...
				if (pModelInst && !pModelInst->IsEmpty() && pModelInst->HasRigidMeshes())
				{
					...
					if (pModel)
					{
						...
						SItemBatchKey key = { pModel, pFirstTex };
						SItemVTFInfo info;
						info.pThingInst = &rThingInst;
	...
	for (auto* pItem : nonBatchable)
	{
		CGraphicThingInstance& rInstance = pItem->ThingInstance;
		rInstance.Render();
		rInstance.BlendRender();
	}
	...
	for (auto& groupPair : vtfGroups)
	{
		...
		std::vector<SItemVTFInfo>& instances = groupPair.second;

		if (instances.size() < 3)
		{
			for (auto& info : instances)
			{
				info.pThingInst->Render();
				info.pThingInst->BlendRender();
			}
			continue;
		}
		...
		// VTF batch DIFFUSE_PNT rigid meshes
...
void CPythonItem::CreateItem(DWORD dwVirtualID, DWORD dwVirtualNumber, float x, float y, float z, bool bDrop)
{
...

	{
		...
		rem.CreateEffect(m_dwDropItemEffectID, Vector3(x, -y, z), Vector3(0,0,0));
	...
	if (bDrop)
	{
		...
		QuaternionMultiply(&pGroundItemInstance->qEnd, &pGroundItemInstance->qEnd, &q);
		...
		Matrix mat;
		MatrixRotationQuaternion(&mat, &pGroundItemInstance->qEnd);
		/*MatrixRotationYawPitchRoll(&mat,
			ToRadian(pGroundItemInstance->rEnd.y),
			ToRadian(pGroundItemInstance->rEnd.x),
			ToRadian(pGroundItemInstance->rEnd.z));*/
...
{
	return (DWORD)(m_NoGradeNameItemData.size());
}
