...
PyObject* sndPlayMusic(PyObject* poSelf, PyObject* poArgs)
{
	...
	// PlayMusic is now FadeInMusic with current volume
...
PyObject* sndSetSoundVolume(PyObject* poSelf, PyObject* poArgs)
{
	...
	// Convert integer grade (0-5) to float (0.0-1.0)
	float fVolume = static_cast<float>(iVolume) / 5.0f;
	SoundEngine::Instance().SetSoundVolume(fVolume);
	return Py_BuildNone();
}
...
{
	// No longer used in new audio engine
	return Py_BuildNone();
}
