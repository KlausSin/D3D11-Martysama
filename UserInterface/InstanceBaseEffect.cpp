...
Color g_akClrTitle[CInstanceBase::TITLE_NUM];
Color g_akClrName[CInstanceBase::NAMECOLOR_NUM];
...
void  CInstanceBase::SetEmpireNameMode(bool isEnable)
{
	...
	if (isEnable)
	{
		g_akClrName[NAMECOLOR_MOB]=g_akClrName[NAMECOLOR_EMPIRE_MOB];
		g_akClrName[NAMECOLOR_NPC]=g_akClrName[NAMECOLOR_EMPIRE_NPC];
		g_akClrName[NAMECOLOR_PC]=g_akClrName[NAMECOLOR_NORMAL_PC];

		for (UINT uEmpire=1; uEmpire<EMPIRE_NUM; ++uEmpire)
			g_akClrName[NAMECOLOR_PC+uEmpire]=g_akClrName[NAMECOLOR_EMPIRE_PC+uEmpire];

	}
	else
	{
		g_akClrName[NAMECOLOR_MOB]=g_akClrName[NAMECOLOR_NORMAL_MOB];
		g_akClrName[NAMECOLOR_NPC]=g_akClrName[NAMECOLOR_NORMAL_NPC];

		for (UINT uEmpire=0; uEmpire<EMPIRE_NUM; ++uEmpire)
			g_akClrName[NAMECOLOR_PC+uEmpire]=g_akClrName[NAMECOLOR_NORMAL_PC];
	}
}

const Color& CInstanceBase::GetIndexedNameColor(UINT eNameColor)
{
	if (eNameColor>=NAMECOLOR_NUM)
	{
		static Color s_kClrNameDefault(0xffffffff);
		return s_kClrNameDefault;
	}

	return g_akClrName[eNameColor];
}
...
void CInstanceBase::ProcessDamage()
{
	...
	int limit = CEffectManager::GetEffectLimit();
	int active = CEffectManager::GetActiveEffectCount();
	if (limit > 0)
	{
		if (active > limit)
		{
		...
		if (active > limit * 60 / 100)
		{
			Vector3 v3MyPos = m_GraphicThingInstance.GetPosition();
			if (CEffectManager::GetDistanceFromPlayer(v3MyPos) > 1500.0f)
			{
				m_DamageQueue.pop();
				return;
			}
	...
	Vector3 v3Rot = Vector3(0.0f, 0.0f, cameraAngle);
	...
	*/
	...
	while(damage>0)
	{
		...
		Matrix matrix,matTrans;
		MatrixIdentity(&matrix);
		matrix._41 = v3Pos.x;
		matrix._42 = v3Pos.y;
		matrix._43 = v3Pos.z;
		MatrixTranslation(&matrix,v3Pos.x,v3Pos.y,v3Pos.z);
		MatrixMultiply(&matrix,&pCamera->GetInverseViewMatrix(),&matrix);
		MatrixTranslation(&matTrans,FONT_WIDTH*index,0,0);
		matTrans._41 = -matTrans._41;
		matrix = matTrans*matrix;
		MatrixMultiply(&matrix,&pCamera->GetViewMatrix(),&matrix);

		rkEftMgr.CreateEffect(ms_adwCRCAffectEffect[rdwCRCEft],Vector3(matrix._41,matrix._42,matrix._43)
			,v3Rot);
...
{
	const Matrix & c_rmatGlobal = m_GraphicThingInstance.GetMatrix();
...
const Color& CInstanceBase::GetNameColor()
{
...
const Color& CInstanceBase::GetTitleColor()
{
	UINT uGrade = GetAlignmentGrade();
	if ( uGrade >= TITLE_NUM)
	{
		static Color s_kClrTitleDefault(0xffffffff);
		return s_kClrTitleDefault;
	}

	return g_akClrTitle[uGrade];
}
...
void CInstanceBase::AttachTextTail()
{
	...
	static Color s_kClrTextTail=Color(1.0f, 1.0f, 1.0f, 1.0f);
	CPythonTextTail::Instance().RegisterCharacterTextTail(m_dwGuildID, dwVID, s_kClrTextTail, fTextTailHeight);
...
{
	//static Color s_kLevelColor = Color(119.0f/255.0f, 246.0f/255.0f, 168.0f/255.0f, 1.0f);
	static Color s_kLevelColor = Color(152.0f/255.0f, 255.0f/255.0f, 51.0f/255.0f, 1.0f);
...
void CInstanceBase::SetEmoticon(UINT eEmoticon)
{
	...
	if (IsPossibleEmoticon())
	{
		...
		Vector3 v3Dir = (pCamera->GetEye()-v3Pos)*9/10;
		...
		//rkEftMgr.CreateEffect(ms_adwCRCAffectEffect[EFFECT_EMOTICON+eEmoticon],v3Pos,Vector3(0,0,0));
...
Color __RGBToColoru(UINT r, UINT g, UINT b)
{
...
bool CInstanceBase::RegisterNameColor(UINT uIndex, UINT r, UINT g, UINT b)
{
	...
	g_akClrName[uIndex]=__RGBToColoru(r, g, b);
	return true;
}
...
bool CInstanceBase::RegisterTitleColor(UINT uIndex, UINT r, UINT g, UINT b)
{
	...
	g_akClrTitle[uIndex]=__RGBToColoru(r, g, b);
	return true;
}
