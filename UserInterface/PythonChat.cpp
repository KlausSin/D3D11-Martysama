...
void CPythonChat::SetChatColor(UINT eType, UINT r, UINT g, UINT b)
{
	...
	m_akClrChat[eType]=Color(dwColor);
}
...
Color & CPythonChat::SChatLine::GetColorRef(DWORD dwID)
{
	assert(dwID < CHAT_LINE_COLOR_ARRAY_MAX_NUM);
...
void CPythonChat::UpdateViewMode(DWORD dwID)
{
	...
	int iLineIndex = (int)(pLineList->size());
	...
	for (itor = pLineList->begin(); itor != pLineList->end();)
	{
		...
		Color & rColor = pChatLine->GetColorRef(dwID);
...
void CPythonChat::ArrangeShowingChat(DWORD dwID)
{
	...
	int icurLineCount = (int)(TempChatLineDeque.size());
...
{
	m_akClrChat[CHAT_TYPE_TALKING]		= Color(1.0f, 1.0f, 1.0f, 1.0f);
	m_akClrChat[CHAT_TYPE_INFO]			= Color(1.0f, 0.785f, 0.785f, 1.0f);
	m_akClrChat[CHAT_TYPE_NOTICE]		= Color(1.0f, 0.902f, 0.730f, 1.0f);
	m_akClrChat[CHAT_TYPE_PARTY]		= Color(0.542f, 1.0f, 0.949f, 1.0f);
	m_akClrChat[CHAT_TYPE_GUILD]		= Color(0.906f, 0.847f, 1.0f, 1.0f);
	m_akClrChat[CHAT_TYPE_COMMAND]		= Color(0.658f, 1.0f, 0.835f, 1.0f);
	m_akClrChat[CHAT_TYPE_SHOUT]		= Color(0.658f, 1.0f, 0.835f, 1.0f);
	m_akClrChat[CHAT_TYPE_WHISPER]		= Color(0xff4AE14A);
	m_akClrChat[CHAT_TYPE_BIG_NOTICE]	= Color(1.0f, 0.902f, 0.730f, 1.0f);
	m_akClrChat[CHAT_TYPE_MONARCH_NOTICE]	= Color(1.0f, 0.902f, 0.730f, 1.0f);
#ifdef ENABLE_DICE_SYSTEM
	m_akClrChat[CHAT_TYPE_DICE_INFO]	= Color(0xFFcc00cc);
#endif
}
...
void CWhisper::AppendChat(int iType, const char * c_szChat)
{
	...
	switch(iType)
	{
		case CPythonChat::WHISPER_TYPE_SYSTEM:
			pChatLine->Instance.SetColor(Color(1.0f, 0.785f, 0.785f, 1.0f));
			break;
		case CPythonChat::WHISPER_TYPE_GM:
			pChatLine->Instance.SetColor(Color(1.0f, 0.632f, 0.0f, 1.0f));
			break;
