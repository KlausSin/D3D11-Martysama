...
typedef struct _MINIMAPVERTEX
{
	...
	DWORD color;      // diffuse color (4 bytes) - DX11: Required for particle shader
	float u, v;       // texcoord (8 bytes)
} MINIMAPVERTEX, * LPMINIMAPVERTEX;  // Total: 24 bytes
#pragma pack(pop)
...
#pragma pack(pop)
	...
	if (!m_VertexBuffer.Create(36, INPUT_LAYOUT_PDT, D3D11_USAGE_DYNAMIC))
	{
		return false;
	}

	if (m_VertexBuffer.Map((void**)&lpOrigMiniMapVertex))
	{
		char* pchMiniMapVertex = (char*)lpOrigMiniMapVertex;
		memset(pchMiniMapVertex, 0, sizeof(char) * 864);
		...
		for (int iY = -3; iY <= 1; ++iY)
		{
			...
			for (int iX = -3; iX <= 1; ++iX)
			{
				...
				lpMiniMapVertex->color = 0xFFFFFFFF;
	...
	if (!m_IndexBuffer.Create(54, DXGI_FORMAT_R16_UINT, D3D11_USAGE_DYNAMIC))
	{
		return false;
	}
...
void CPythonMiniMap::__SetPosition()
{
	...
	{
		m_matMiniMapCover._11 = 1.0f / m_fWidth;
		m_matMiniMapCover._22 = 1.0f / m_fHeight;
		m_matMiniMapCover._41 = -m_fScreenX / m_fWidth;
		m_matMiniMapCover._42 = -m_fScreenY / m_fHeight;
	}
...
void CPythonMiniMap::RegisterAtlasMark(BYTE byType, const char* c_szName, long lx, long ly)
{
	...
	case CActorInstance::TYPE_WARP:
		...
		{
			int iPos = (int)(aAtlasMarkInfo.m_strText.find(" "));
			if (iPos >= 0)
...
void CPythonMiniMap::__Initialize()
{
	...
	MatrixIdentity(&m_matIdentity);
	MatrixIdentity(&m_matWorld);
	MatrixIdentity(&m_matMiniMapCover);
	MatrixIdentity(&m_matWorldAtlas);
}
