...
bool CEffectData::LoadScript(const char * c_szFileName)
{
	...
	int iPos = (int)(strNoExtensionName.find(strPathHeader.c_str()));
	if (iPos >= 0)
...
{
	return (DWORD)(m_LightVector.size());
}
...
{
	return (DWORD)(m_ParticleVector.size());
}
...
{
	return (DWORD)(m_MeshVector.size());
}
...
Vector3 CEffectData::GetBoundingSpherePosition()
{
