...
#include "../EterLib/GrpStateEnum.h"

#define Clamp(x, min, max)  x = (x<min  ? min : x<max ? x : max);
#define GRAVITY			Vector3(0.0f, 0.0f, -9.8f)

#define	MAX_FRAME		20
#define MAX_TEXTURE		20

// Effect vertex types
typedef	struct SEffectPointVertex
{
	float	x, y, z;
} TEffectPointVertex;

typedef	struct SEffectPTVertex
{
	float	x, y, z;
	float	tu, tv;
} TEffectPTVertex;

typedef	struct SEffectPDTVertex
{
	float	x, y, z;
	DWORD	color;
	float	tu, tv;
...
inline TEffectPDTVertex MakeEffectPDTVertex(float x, float y, float z, DWORD dif, float u, float v)
{
	TEffectPDTVertex result;
