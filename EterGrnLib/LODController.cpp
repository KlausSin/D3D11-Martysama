...
void CGrannyLODController::SetLODLevel(BYTE bLodLevel)
{
	...
	if (m_que_pkModelInst.size() > 0)
		m_bLODLevel	= (BYTE) MIN((int)(m_que_pkModelInst.size() - 1), bLodLevel);
}
...
{
	UpdateLODLevel(fDistanceFromCenter * fDistanceFromCenter, fDistanceFromCamera * fDistanceFromCamera);
	UpdateTime(fElapsedTime);
}

void CGrannyLODController::UpdateLODLevel(float fDistanceFromCenterSq, float fDistanceFromCameraSq)
{
	...
	float fDistanceFromCenter = sqrtf(fDistanceFromCenterSq);
	float fDistanceFromCamera = sqrtf(fDistanceFromCameraSq);
	...
	if (fDistanceFromCenter > LOD_APPLY_MIN)
	{
		...
		if (m_fLODDistance>0.0f)
			m_dwLODAniFPS = (DWORD) ((CGrannyModelInstance::ANIFPS_MAX - CGrannyModelInstance::ANIFPS_MIN) * fLODFactor / m_fLODDistance + (float)CGrannyModelInstance::ANIFPS_MIN);
		else
...
void CGrannyLODController::UpdateSkeleton(const Matrix * c_pWorldMatrix, float fElapsedTime)
{
	if (m_pCurrentModelInstance)
		m_pCurrentModelInstance->UpdateSkeleton(c_pWorldMatrix, fElapsedTime);
}

void CGrannyLODController::DeformAll(const Matrix * c_pWorldMatrix)
{
	std::deque<CGrannyModelInstance *>::iterator i;
	for (i=m_que_pkModelInst.begin(); i!=m_que_pkModelInst.end(); ++i)
	{
...
void CGrannyLODController::DeformNoSkin(const Matrix * c_pWorldMatrix)
{
	if (m_pCurrentModelInstance)
		m_pCurrentModelInstance->DeformNoSkin(c_pWorldMatrix);
}

void CGrannyLODController::Deform(const Matrix * c_pWorldMatrix)
{
	if (m_pCurrentModelInstance)
...
void CGrannyLODController::GetBoundBox(Vector3 * vtMin, Vector3 * vtMax)
{
	if (m_pCurrentModelInstance)
		m_pCurrentModelInstance->GetBoundBox(vtMin, vtMax);
}

bool CGrannyLODController::Intersect(const Matrix * c_pMatrix, float * u, float * v, float * t)
{
	if (!m_pCurrentModelInstance)
		return false;
