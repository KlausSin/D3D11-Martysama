...
class CGrannyLODController : public CGraphicBase
{
	...
	public:
		...
		{
			float fDistanceFromCenterSq;
			float fDistanceFromCameraSq;

			void operator() (CGrannyLODController * pController)
			{
				if (pController->isModelInstance())
					pController->UpdateLODLevel(fDistanceFromCenterSq, fDistanceFromCameraSq);
			}
		};
		...
		struct FBoundBox
		{
			...
			FBoundBox(Vector3 * vtMin, Vector3 * vtMax)
			{
				m_vtMin = vtMin;
				m_vtMax = vtMax;
			}
		...
		void	UpdateLODLevel(float fDistanceFromCenterSq, float fDistanceFromCameraSq);
		void	UpdateTime(float fElapsedTime);

		void	UpdateSkeleton(const Matrix * c_pWorldMatrix, float fElapsedTime);
		void	Deform(const Matrix * c_pWorldMatrix);
		void	DeformNoSkin(const Matrix * c_pWorldMatrix);
		void	DeformAll(const Matrix * c_pWorldMatrix);
		...
		bool	Intersect(const Matrix * c_pMatrix, float * u, float * v, float * t);
		...
		CGrannyModelInstance*	GetCurrentModelInstance() const { return m_pCurrentModelInstance; }
