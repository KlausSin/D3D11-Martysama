...
{
	return m_Initialized && ma_sound_is_playing(&m_Sound) == MA_TRUE;
}
...
bool MaSoundInstance::Play()
{
	if (!m_Initialized)
	{
		TraceError("MaSoundInstance::Play - not initialized");
		return false;
	}

	ma_result seekResult = ma_sound_seek_to_pcm_frame(&m_Sound, 0);
	if (seekResult != MA_SUCCESS)
	{
		TraceError("MaSoundInstance::Play - seek failed: %d", seekResult);
		return false;
	}

	ma_result startResult = ma_sound_start(&m_Sound);
	if (startResult != MA_SUCCESS)
	{
		TraceError("MaSoundInstance::Play - start failed: %d", startResult);
		return false;
	}
