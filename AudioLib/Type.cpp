...
bool NSound::LoadSoundInformationPiece(const char* c_szFileName, NSound::TSoundDataVector& rSoundDataVector, const char* c_szPathHeader)
{
	...
	{
		SetResultString((strFileName + " Can not open file for reading").c_str());
		return false;
	}
	...
	{
		SetResultString((strFileName + " File format error, SoundDataCount Unable to find.").c_str());
		return false;
	}
	...
	for (uint32_t i = 0; i < rSoundDataVector.size(); ++i)
	{
		...
		{
			SetResultString((strFileName + " File format error: " + szSoundDataHeader + " Unable to find").c_str());
			return false;
		}

		if (2 != pTokenVector->size())
		{
			SetResultString((strFileName + " File format error: The size of the vector is not 2").c_str());
			return false;
		}
	...
	SetResultString((strFileName + " Loaded").c_str());
	return true;
}
...
bool NSound::SaveSoundInformationPiece(const char* c_szFileName, NSound::TSoundDataVector& rSoundDataVector)
{
	...
	if (!File)
	{
		...
		SetResultString((strFileName + " Cannot open file for writing").c_str());
		return false;
	}
