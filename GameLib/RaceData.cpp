...
static Mutex s_kRaceDataPoolMutex;  // Thread safety for async race loading
...
{
	s_kRaceDataPoolMutex.Lock();
	CRaceData* pData = ms_kPool.Alloc();
	s_kRaceDataPoolMutex.Unlock();
	return pData;
}
...
{
	return (DWORD)(m_AttachingDataVector.size());
}
