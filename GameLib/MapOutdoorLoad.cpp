...
bool CMapOutdoor::Load(float x, float y, float z)
{
	...
	CSpeedTreeForestDX11::Instance().SetRenderingDevice(ms_pDevice);
...
bool CMapOutdoor::LoadSetting(const char * c_szFileName)
{
	...
	MatrixScaling(&m_matStaticShadow,
	...
	MatrixScaling(&m_matDynamicShadowScale, 1.0f / SHADOW_PROJECTION_SIZE, -1.0f / SHADOW_PROJECTION_SIZE, 1.0f);
	m_matDynamicShadowScale._41 = 0.5f;
	m_matDynamicShadowScale._42 = 0.5f;

	// Transform
	MatrixScaling(&m_matBuildingTransparent, 1.0f / ((float)ms_iWidth), -1.0f / ((float)ms_iHeight), 1.0f);
