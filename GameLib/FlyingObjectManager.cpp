...
CFlyingInstance * CFlyingManager::CreateFlyingInstanceFlyTarget(const DWORD dwID, const Vector3 & v3StartPosition, const CFlyTarget & cr_FlyTarget, bool canAttack)
{
...
void CFlyingManager::CreateIndexedFly(DWORD dwIndex, CActorInstance * pStartActor, CActorInstance * pEndActor)
{
	...
	switch (rIndexFlyData.byType)
	{
		...
		case INDEX_FLY_TYPE_FIRE_CRACKER:
		{
			...
			float fxRand = fDistance * cosf(ToRadian(fRot));
			float fyRand = fDistance * sinf(ToRadian(fRot));
...
CFlyingInstance * CFlyingManager::CreateIndexedFlyingInstanceFlyTarget(const DWORD dwIndex, const Vector3 & v3StartPosition, const CFlyTarget & cr_FlyTarget)
{
	if (m_kMap_dwIndexFlyData.end() == m_kMap_dwIndexFlyData.find(dwIndex))
	{
	...
	return CreateFlyingInstanceFlyTarget(rIndexFlyData.dwCRC, Vector3(v3StartPosition.x, v3StartPosition.y, v3StartPosition.z), cr_FlyTarget, true);
}
