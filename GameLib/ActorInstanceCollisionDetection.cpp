...
void CActorInstance::UpdatePointInstance(TCollisionPointInstance * pPointInstance)
{
	...
	for (;sit!=pPointInstance->c_pCollisionData->SphereDataVector.end();++sit,++dit)
	{
		...
		Matrix matPoint;
		MatrixTranslation(&matPoint, c.v3Position.x, c.v3Position.y, c.v3Position.z);
...
void CActorInstance::UpdateAdvancingPointInstance()
{
	...
	for (; itor != m_BodyPointInstanceList.end(); ++itor)
	{
		...
		if (rInstance.isAttached)
		{
			...
			Matrix* pMatBone = (Matrix*)pModelInstance->GetBoneMatrixPointer(rInstance.dwBoneIndex);
...
#else
			matCenter = *(Matrix*)pModelInstance->GetBoneMatrixPointer(rInstance.dwBoneIndex);
#endif
...
bool CActorInstance::CheckCollisionDetection(const CDynamicSphereInstanceVector * c_pAttackingSphereVector, Vector3 * pv3Position)
{
...
BOOL CActorInstance::__SplashAttackProcess(CActorInstance & rVictim)
{
	...
	float fDistance = Vec3LengthSq(&v3Distance);
	...
	if (rVictim.CheckCollisionDetection(&m_kSplashArea.SphereInstanceVector, &v3HitPosition))
	{
		...
		int iCurrentHitCount = (int)(rHittedInstanceMap.size());
...
BOOL CActorInstance::__NormalAttackProcess(CActorInstance & rVictim)
{
	...
	// Hoist rotation trig outside hit data loop — rotation doesn't change per victim
	float c = cosf(ToRadian(GetRotation()));
	float s = sinf(ToRadian(GetRotation()));
				...
				for(dsit = c_DefendingSphereVector.begin(); dsit!= c_DefendingSphereVector.end();++dsit, ++index)
				{
					...
					if (DetectCollisionDynamicZCylinderVSDynamicZCylinder(dsi, sub))
					{
						...
						else
						{
							...
							int iCurrentHitCount = (int)(itHitData->second.size());
...
BOOL CActorInstance::TestPhysicsBlendingCollision(CActorInstance & rVictim)
{
	...
	Vector3 v3Distance = Vector3(rVictim.m_x - kPPosLast.x, rVictim.m_y - kPPosLast.y, rVictim.m_z - kPPosLast.z);
	float fDistance = Vec3LengthSq(&v3Distance);
