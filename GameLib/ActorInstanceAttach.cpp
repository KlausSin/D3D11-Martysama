...
DWORD CActorInstance::AttachEffectByID(DWORD dwParentPartIndex, const char * c_pszBoneName, DWORD dwEffectID, const Vector3 * c_pv3Position)
{
	CEffectManager& rkEftMgr=CEffectManager::Instance();

#ifdef ENABLE_EFFECT_LIMIT
	// Early-out when effect system is at capacity — avoid creating zombie entries
	if (CEffectManager::GetEffectLimit() > 0 &&
		CEffectManager::GetActiveEffectCount() >= CEffectManager::GetEffectLimit())
		return 0;
#endif
	...
	if (c_pv3Position)
	{
		MatrixTranslation(&ae.matTranslation, c_pv3Position->x, c_pv3Position->y, c_pv3Position->z);
	}
	else
	{
		MatrixIdentity(&ae.matTranslation);
	}
...
void CActorInstance::UpdateAttachingInstances()
{
	...
	for (it = m_AttachingEffectList.begin(); it!= m_AttachingEffectList.end();)
	{
		...
		bool bExpired = (EFFECT_LIFE_NORMAL == it->iLifeType && it->dwEndTime < dwCurrentTime);
		if (bExpired || !rkEftMgr.SelectEffectInstance(it->dwEffectIndex))
		{
