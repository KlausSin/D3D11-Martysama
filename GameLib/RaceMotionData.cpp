...
static Mutex s_kRaceMotionDataPoolMutex;  // Thread safety for async race loading
...
{
	s_kRaceMotionDataPoolMutex.Lock();
	CRaceMotionData* pData = ms_kPool.Alloc();
	s_kRaceMotionDataPoolMutex.Unlock();
	return pData;
}
...
{
	return (DWORD)(m_MotionEventDataVector.size());
}
