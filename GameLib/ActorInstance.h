...
class CActorInstance : public IActorInstance, public IFlyTargetableObject
{
	...
	public:
		...
		DWORD AttachEffectByID(DWORD dwParentPartIndex, const char * c_pszBoneName, DWORD dwEffectID, const Vector3 * c_pv3Position = NULL);
		...
		bool CheckCollisionDetection(const CDynamicSphereInstanceVector * c_pAttackingSphereVector, Vector3 * pv3Position);
	...
	public:
		...
		float		GetAlphaValue();
		bool		IsBlendingAlpha() const { return m_kBlendAlpha.m_isBlending; }
		bool		IsAlphaBlendOverdue() const;
		void		CompleteAlphaBlend();
		...
		void		SetAddColor(const Color & c_rColor);
		...
		int			GetRenderMode() const { return m_iRenderMode; }

#ifdef ENABLE_RENDER_MODE_GROUPING
		// Batch rendering functions
		static void BeginBatchRender();
		static void EndBatchRender();
		void RenderWithOneTexture();
		void BlendRenderWithOneTexture();
		DWORD GetAddColorDWORD() { return (DWORD)m_AddColor; }
#endif
	...
	protected:
		...
		void __ProcessDataAttackSuccess(const NRaceData::TAttackData & c_rAttackData, CActorInstance & rVictim, const Vector3 & c_rv3Position, UINT uiSkill = 0, BOOL isSendPacket = TRUE);
	...
	protected:
		...
		Color					m_AddColor;
		float						m_fAlphaValue;
		bool						m_bSavedAlphaTest;
		DWORD						m_dwSavedAlphaRef;
